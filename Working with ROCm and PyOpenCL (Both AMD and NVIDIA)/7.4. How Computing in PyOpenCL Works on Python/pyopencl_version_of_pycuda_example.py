...
#Initialize three PyOpenCL arrays all with zeroes
a_gpu = clarray.to_device(command_queue, np.zeros(N).astype(np.double))
b_gpu = clarray.to_device(command_queue, np.zeros(N).astype(np.double))

#Update the first two arrays with values 23 and 12 respectively
a_gpu.fill(23.0)
b_gpu.fill(12.0)

event = multiply(a_gpu, b_gpu)
...
#Randomly choose index from second array to confirm its updated changes
print("New value of third array element with random index", random, "is", b_gpu[random])